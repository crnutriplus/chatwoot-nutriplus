...
RSpec.describe Instagram::Messenger::SendOnInstagramService do
  ...
  it 'adds top-level reply_to on the graph.facebook.com Instagram Messenger route' do
    ...
    expect(message.reload.content_attributes['in_reply_to_external_id']).to eq('mid.original')
    expect(message.source_id).to eq('mid.sent')
  end
